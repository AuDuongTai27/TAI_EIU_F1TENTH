...
class TreeViz:
    ...
    def append(self, node):
        if node.parent is None:
            ...
            # SỬA LỖI: Thay node.coordinates bằng [node.x, node.y]
            node_coords = self.grid_to_world_coord([node.x, node.y])
            
            self.nodes_marker.points.append(Point(x=node_coords[0], y=node_coords[1], z=0.1))
       
        else:
            # Convert from grid to world coordinate
            # SỬA LỖI: Thay node.parent.coordinates bằng [node.parent.x, node.parent.y]
            start_point = self.grid_to_world_coord([node.parent.x, node.parent.y])
            
            # SỬA LỖI: Thay node.coordinates bằng [node.x, node.y]
            end_point = self.grid_to_world_coord([node.x, node.y])
            ...
            # (Optional: Có thể bỏ vòng lặp while này nếu nó gây lag, hoặc giữ nguyên nếu muốn chắc chắn)
